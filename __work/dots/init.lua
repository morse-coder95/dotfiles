...
Plug 'klen/nvim-test'  -- run tests with <leader>t commands
...
Plug('zbirenbaum/copilot.lua', { ['commit'] = '73dbc0e' })  -- Copilot integration (pinned to last Node 18 compatible version, before LSP binaries)
...
local pytest_runner = require('nvim-test.runners.pytest')
pytest_runner:setup {
    args = { "--verbose", "--disable-pytest-warnings", "--capture=no" }
}
-- Override build_test_args to use -k for TestNearest (works with parameterized tests)
function pytest_runner:build_test_args(args, tests)
    table.insert(args, "-k")
    table.insert(args, table.concat(tests, " and "))
end
...
-- Copilot setup with inline suggestions (ghost text)
require('copilot').setup({
    suggestion = {
        enabled = true,
        auto_trigger = true,
        keymap = {
            accept = "<C-y>",  -- Ctrl+M to accept suggestion
        },
    },
    panel = { enabled = false },
})
...
cmp.setup({
    ...
    mapping = cmp.mapping.preset.insert({
        ['<C-n>'] = cmp.config.disable,  -- disable to let UltiSnips handle it
        ['<C-p>'] = cmp.config.disable,  -- disable to let UltiSnips handle it
...
wk.add({
    ...
    { "<leader>ffp", "<cmd>FzfLua files cwd=~/source/python<CR>", desc = "python" },
    { "<leader>ffs", "<cmd>FzfLua files cwd=~/source/<CR>", desc = "source" },
    { "<leader>ffd", "<cmd>FzfLua files cwd=~/db/<CR>", desc = "db" },
    { "<leader>ffc", "<cmd>FzfLua files cwd=~/cpp/<CR>", desc = "cpp" },
    { "<leader>ffw", "<cmd>FzfLua files cwd=~/web/<CR>", desc = "web" },
    ...
    { "<leader>fwp", "<cmd>FzfLua live_grep cwd=~/source/python<CR>", desc = "python" },
    { "<leader>fws", "<cmd>FzfLua live_grep cwd=~/source/<CR>", desc = "source" },
    { "<leader>fwd", "<cmd>FzfLua live_grep cwd=~/db/<CR>", desc = "db" },
    { "<leader>fwc", "<cmd>FzfLua live_grep cwd=~/cpp/<CR>", desc = "cpp" },
    { "<leader>fww", "<cmd>FzfLua live_grep cwd=~/web/<CR>", desc = "web" },
